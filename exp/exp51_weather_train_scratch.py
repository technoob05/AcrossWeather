# =============================================================================
# EXP51: Weather-Augmented Training FROM SCRATCH + Robustness Evaluation
# =============================================================================
# Copy to Kaggle notebook → Run All → Done
#
# Trains SPDGeo-DPEA-MAR from scratch (no EXP35 checkpoint) on
# weather-augmented SUES-200 training data, then evaluates
# drone→satellite retrieval under 10 weather conditions.
#
# Training strategy:
#   - Fresh model init (DINOv2 ViT-S/14 pretrained backbone + random heads)
...
#   - 120 epochs (full training, same as EXP35)
#   - Original LR schedule (3e-4 / 3e-5)
#
# Evaluation:
#   - Per-weather R@1/R@5/R@10/mAP
#   - Per-altitude breakdown
#   - Weather × altitude cross table
#   - Comparison with EXP49 (zero-shot) and EXP50 (fine-tuned)
...
class Config:
    ...
    # --- Training (FROM SCRATCH — same hyperparams as EXP35) ---
    NUM_EPOCHS      = 120         # Full training (same as EXP35)
    P_CLASSES       = 16
    K_SAMPLES       = 4
    LR              = 3e-4        # Original LR (not fine-tune LR)
    BACKBONE_LR     = 3e-5        # Original backbone LR
    WEIGHT_DECAY    = 0.01
    WARMUP_EPOCHS   = 5
    USE_AMP         = True
    SEED            = 42
...
def train_one_epoch(model, teacher, ema, loader, losses, new_losses, optimizer,
    ...
    # Reconstruction warmup: ramp from 0 to full weight
...
def generate_latex_table(all_results, normal_results):
    ...
    lines.append(r"\caption{Training from scratch on weather-augmented SUES-200 (Drone$\to$Satellite).}")
    lines.append(r"\label{tab:weather_scratch_training}")
...
def print_comparison(all_results):
    print(f"\n{'='*100}")
    print(f"  COMPARISON: EXP49 (zero-shot) vs EXP50 (fine-tune) vs EXP51 (from scratch)")
    print(f"{'='*100}")
    print(f"  {'Weather':>12s}  {'EXP49':>8s}  {'EXP50':>8s}  {'EXP51':>8s}  "
          f"{'Δ49→51':>8s}  {'Δ50→51':>8s}")
    print(f"  {'-'*58}")
    for w in CFG.WEATHER_NAMES:
        e49 = EXP49_RESULTS[w]['R@1']
        e50 = EXP50_RESULTS[w]['R@1']
        e51 = all_results[w]['overall']['R@1']
        d49 = e51 - e49; d50 = e51 - e50
        print(f"  {w:>12s}  {e49:7.2f}%  {e50:7.2f}%  {e51:7.2f}%  "
              f"{d49:+7.2f}%  {d50:+7.2f}%")
    print(f"  {'-'*58}")
    adv = [w for w in CFG.WEATHER_NAMES if w != 'normal']
    e49_adv = np.mean([EXP49_RESULTS[w]['R@1'] for w in adv])
    e50_adv = np.mean([EXP50_RESULTS[w]['R@1'] for w in adv])
    e51_adv = np.mean([all_results[w]['overall']['R@1'] for w in adv])
    print(f"  {'Avg(adverse)':>12s}  {e49_adv:7.2f}%  {e50_adv:7.2f}%  {e51_adv:7.2f}%  "
          f"{e51_adv-e49_adv:+7.2f}%  {e51_adv-e50_adv:+7.2f}%")
    ...
    e51_all = np.mean([all_results[w]['overall']['R@1'] for w in CFG.WEATHER_NAMES])
    print(f"  {'Avg(all)':>12s}  {e49_all:7.2f}%  {e50_all:7.2f}%  {e51_all:7.2f}%  "
          f"{e51_all-e49_all:+7.2f}%  {e51_all-e50_all:+7.2f}%")
    print(f"{'='*100}")
...
def main():
    ...
    print("=" * 75)
    print("  EXP51: Weather-Augmented Training FROM SCRATCH")
    ...
    # ---- 4. Training Loop ----
    print(f"\n[4/5] Training from scratch on weather data ({CFG.NUM_EPOCHS} epochs) …")
    best_r1 = 0.0
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            ...
            if avg_eval_r1 > best_avg_weather_r1:
                ...
                }, os.path.join(CFG.OUTPUT_DIR, 'exp51_weather_scratch_best.pth'))
    ...
    best_ckpt_path = os.path.join(CFG.OUTPUT_DIR, 'exp51_weather_scratch_best.pth')
    ...
    print(f"\n{'='*75}")
    print(f"  EXP51 COMPLETE — Weather-Augmented Training FROM SCRATCH")
    print(f"{'='*75}")
