# =============================================================================
# EXP53: Weather Training FROM SCRATCH with ONLINE Augmentation (WeatherPrompt-style)
# =============================================================================
# Copy to Kaggle notebook → Run All → Done
#
# Trains SPDGeo-DPEA-MAR from scratch using WeatherPrompt's exact data
...
#
# Key difference from EXP51:
#   - EXP51: used 4800 pre-generated weather images (120 loc × 4 alt × 10 weathers)
#   - EXP53: uses original drone images, weather applied on-the-fly
...
#   (NO model checkpoint needed — training from scratch)
# =============================================================================
...
class Config:
    ...
    # --- Training (FROM SCRATCH + online augmentation) ---
    NUM_EPOCHS      = 120         # Full training (same as EXP35/EXP51)
    P_CLASSES       = 16
    K_SAMPLES       = 4
    LR              = 3e-4        # Original LR (not fine-tune)
    BACKBONE_LR     = 3e-5
    WEIGHT_DECAY    = 0.01
    WARMUP_EPOCHS   = 5
    USE_AMP         = True
...
class OnlineWeatherTrainDataset(Dataset):
    ...
    def __init__(self, sues_root, transform=None, weather_augmenters=None):
        ...
        # drone_by_location for compatibility
    ...
    def __getitem__(self, idx):
        ...
        # Apply random weather augmentation (WeatherPrompt-style)
...
class PKSamplerOnline:
    """
    PK-sampler for OnlineWeatherTrainDataset where each index is a location.
    Yields batches of P locations × K repeated accesses.
...
def generate_latex_table(all_results, normal_results):
    ...
    lines.append(r"\caption{Training from scratch with online WeatherPrompt-style augmentation (Drone$\to$Satellite).}")
    lines.append(r"\label{tab:weather_scratch_online}")
...
# =============================================================================
# COMPARISON WITH EXP49 + EXP50 + EXP51 + EXP52
# =============================================================================
EXP49_RESULTS = {
...
def print_comparison(all_results):
    print(f"\n{'='*130}")
    print(f"  COMPARISON: EXP49 (zero-shot) vs EXP50 (ft-pregen) vs "
          f"EXP51 (scratch-pregen) vs EXP53 (scratch-online)")
    print(f"{'='*130}")
    print(f"  {'Weather':>12s}  {'EXP49':>8s}  {'EXP50':>8s}  {'EXP51':>8s}  {'EXP53':>8s}  "
          f"{'Δ50→53':>8s}  {'Δ51→53':>8s}")
    print(f"  {'-'*70}")
    for w in CFG.WEATHER_NAMES:
        e49 = EXP49_RESULTS[w]['R@1']
        e50 = EXP50_RESULTS[w]['R@1']
        e51 = EXP51_RESULTS[w]['R@1']
        e53 = all_results[w]['overall']['R@1']
        d50 = e53 - e50; d51 = e53 - e51
        print(f"  {w:>12s}  {e49:7.2f}%  {e50:7.2f}%  {e51:7.2f}%  {e53:7.2f}%  "
              f"{d50:+7.2f}%  {d51:+7.2f}%")
    print(f"  {'-'*70}")
    adv = [w for w in CFG.WEATHER_NAMES if w != 'normal']
    ...
    e53_adv = np.mean([all_results[w]['overall']['R@1'] for w in adv])
    print(f"  {'Avg(adverse)':>12s}  {e49_adv:7.2f}%  {e50_adv:7.2f}%  {e51_adv:7.2f}%  {e53_adv:7.2f}%  "
          f"{e53_adv-e50_adv:+7.2f}%  {e53_adv-e51_adv:+7.2f}%")
    ...
    e53_all = np.mean([all_results[w]['overall']['R@1'] for w in CFG.WEATHER_NAMES])
    print(f"  {'Avg(all)':>12s}  {e49_all:7.2f}%  {e50_all:7.2f}%  {e51_all:7.2f}%  {e53_all:7.2f}%  "
          f"{e53_all-e50_all:+7.2f}%  {e53_all-e51_all:+7.2f}%")
    print(f"{'='*130}")
...
def main():
    ...
    print("=" * 75)
    print("  EXP53: Weather Training FROM SCRATCH with ONLINE Augmentation")
    print(f"  SUES-200:     {CFG.SUES_ROOT}")
    print(f"  Device:       {DEVICE}")
    print(f"  Epochs:       {CFG.NUM_EPOCHS} (from scratch)")
    ...
    # ---- 1. Build Model (from scratch — NO checkpoint) ----
    print("\n[1/5] Initializing model from scratch …")
    model = SPDGeoDPEAMARModel(CFG.NUM_CLASSES).to(DEVICE)
    print("  Model initialized (DINOv2 backbone pretrained, heads random)")
    ...
    # Optimizer (original LR — training from scratch)
    ...
    # ---- 4. Training Loop ----
    print(f"\n[4/5] Training from scratch with online weather augmentation ({CFG.NUM_EPOCHS} epochs) …")
    best_r1 = 0.0
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            ...
            if avg_eval_r1 > best_avg_weather_r1:
                ...
                }, os.path.join(CFG.OUTPUT_DIR, 'exp53_weather_scratch_online_best.pth'))
    ...
    best_ckpt_path = os.path.join(CFG.OUTPUT_DIR, 'exp53_weather_scratch_online_best.pth')
    ...
    adv_weathers = [w for w in CFG.WEATHER_NAMES if w != 'normal']
    summary = {
        'experiment': 'EXP53_Weather_Scratch_Online',
        'model': 'SPDGeo-DPEA-MAR (from scratch with online weather augmentation)',
        'dataset': 'SUES-200',
    ...
    json_path = os.path.join(CFG.OUTPUT_DIR, 'exp53_weather_scratch_online.json')
    ...
    print(f"\n{'='*75}")
    print(f"  EXP53 COMPLETE — From Scratch with Online Weather Augmentation")
    print(f"{'='*75}")
