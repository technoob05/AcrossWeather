# =============================================================================
# EXP60: Weather Training from Scratch — University-1652
# =============================================================================
# Copy to Kaggle notebook → Run All → Done
#
# Trains from scratch (random init + DINOv2 pretrained backbone)
...
#
# KEY DIFFERENCES from EXP59 (fine-tune):
#   - EXP59: starts from GeoPartLoss best checkpoint (98.4% SUES-200)
#   - EXP60: starts from RANDOM INIT (only DINOv2 backbone pretrained)
#   - Requires longer training: 120 epochs (vs 60 for fine-tune)
#   - Requires RECON_WARMUP: 10 epochs (cold-start for masked reconstruction)
#   - Higher base LR: 3e-4 (vs 1e-4 for fine-tune)
#
# KEY DIFFERENCES from EXP57 (SUES-200 scratch):
#   - EXP57: 200 locations × 4 altitudes, WITH altitude conditioning (FiLM)
#   - EXP60: 701 buildings, NO altitude conditioning, Uni-1652 dataset
#   - EXP60 uses 3-group GeoPartLoss (no altitude group)
#
...
class Config:
    ...
    EXPERIMENT_NAME = "EXP60_Scratch_Uni1652_OnlineAug"
    ...
    # Training params (scratch — higher LR, more epochs)
    NUM_EPOCHS      = 120
    P_CLASSES       = 16       # building classes per batch
    K_SAMPLES       = 4        # drone images per building per step
    LR              = 3e-4
    BACKBONE_LR     = 3e-5
    WEIGHT_DECAY    = 0.01
    WARMUP_EPOCHS   = 5
    USE_AMP         = True
    SEED            = 42
    RECON_WARMUP    = 10       # warmup needed — training from scratch
...
# =============================================================================
# DATASET — University-1652 Online Weather Training (identical to EXP59)
# =============================================================================
class Uni1652OnlineWeatherTrainDataset(Dataset):
    """
    University-1652 training with WeatherPrompt-style online augmentation.
    1 random drone image per building per epoch, random weather on-the-fly.
    """
    def __init__(self, root, transform=None):
        self.root = root
        self.drone_tf = transform or get_transforms("train")
        self.sat_tf = get_transforms("test")
        ...
        self.drone_by_building = {}; self.sat_by_building = {}
        for bid in self.building_ids:
            dp = os.path.join(drone_dir, bid); sp = os.path.join(sat_dir, bid)
        ...
        print(f"  [Uni-1652 Train] {len(self.valid_bids)} buildings | "
              f"{self.num_classes} classes | Online weather aug (SCRATCH)")
...
class MaskedPartReconstruction(nn.Module):
    ...
    def forward(self, projected_patches, part_features, assignment):
        B, N, D = projected_patches.shape; num_mask = int(N * self.mask_ratio)
        noise = torch.rand(B, N, device=projected_patches.device)
        ids_shuffle = noise.argsort(dim=1); mask_indices = ids_shuffle[:, :num_mask]
...
class SPDGeoDPEMARModel(nn.Module):
    def __init__(self, num_classes, cfg=CFG):
        ...
        print(f"  SPDGeo-DPE-MAR (scratch): {total/1e6:.1f}M trainable parameters")
...
class ProxyAnchorLoss(nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        has_pos = one_hot.sum(0) > 0; pos_term = torch.log(1 + pos_exp.sum(0))
...
class EMADistillationLoss(nn.Module):
    def forward(self, student_emb, ema_emb): return (1 - F.cosine_similarity(student_emb, ema_emb)).mean()
...
class GeoPartLoss(nn.Module):
    def __init__(self, num_classes, embed_dim, num_groups=3, cfg=None):
        ...
        self.ema_dist = EMADistillationLoss(); self.diversity = PrototypeDiversityLoss()
    ...
    def forward(self, d_out, s_out, labels, model, ema, drone, sat, epoch, device):
        ...
        loss_dict.update({'recon': l_recon.item() if torch.is_tensor(l_recon) else l_recon,
                          'div': l_div.item()})
        groups = [L_align, L_ema, L_part]; group_names = ['align', 'ema', 'part']
        ...
        for i, (L, name) in enumerate(zip(groups, group_names)):
            ...
            group_weights[name] = precision.item(); loss_dict[f'G_{name}'] = L.item()
...
# =============================================================================
@torch.no_grad()
def extract_features(model, paths, labels, weather_condition="normal"):
    model.eval()
    tf = get_transforms("test")
    aug = WEATHER_AUGMENTERS.get(weather_condition) if weather_condition != "normal" else None
    feats = []; lbs = []; batch_imgs = []; batch_labels = []
    for i, (path, label) in enumerate(zip(paths, labels)):
        try:
            img_np = np.array(Image.open(path).convert('RGB'))
            if aug is not None: img_np = aug.augment_image(img_np)
...
def evaluate_weather(model, uni_root, weather_subset=None):
    ...
    for bid in sorted(os.listdir(gallery_dir)):
        ...
        for f in sorted(os.listdir(bp)):
            if f.endswith(('.jpg', '.jpeg', '.png')):
                gallery_paths.append(os.path.join(bp, f)); gallery_labels.append(int(bid))
    ...
    for bid in sorted(os.listdir(query_dir)):
        ...
        for f in sorted(os.listdir(bp)):
            if f.endswith(('.jpg', '.jpeg', '.png')):
                query_paths.append(os.path.join(bp, f)); query_labels.append(int(bid))
    gallery_feats, g_labels = extract_features(model, gallery_paths, gallery_labels, "normal")
    results = {}; r1_list = []
    for cond in conditions:
        drone_feats, d_labels = extract_features(model, query_paths, query_labels, cond)
        m = compute_metrics(drone_feats, d_labels, gallery_feats, g_labels)
        results[cond] = m; r1_list.append(m['R@1'])
...
def main():
    ...
    print(f"  Train from Scratch on University-1652 | Online Weather Aug")
    print(f"  {CFG.NUM_EPOCHS} epochs | IMG: {CFG.IMG_SIZE} | UNFREEZE: {CFG.UNFREEZE_BLOCKS}")
    print(f"  GeoPartLoss 3-group | RECON_WARMUP: {CFG.RECON_WARMUP}")
    print("=" * 75)
    ...
    # ---- Model (from scratch) ----
    print("\n[MODEL] Building SPDGeo-DPE-MAR from scratch (no checkpoint)...")
    ...
    print(f"  RECON_WARMUP: epoch {CFG.RECON_WARMUP} (reconstruction loss starts after warmup)")
    ...
    # ---- Training Loop ----
    best_avg_adverse = 0.0; best_ema_adverse = 0.0; results_log = []
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            ...
            entry = {'epoch': epoch, 'normal_r1': normal_r1,
                     'avg_all_r1': avg_all, 'avg_adv_r1': avg_adv,
                     'results': {c: eval_results[c]['R@1'] for c in eval_results}}
