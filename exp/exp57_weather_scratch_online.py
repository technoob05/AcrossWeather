# =============================================================================
# EXP57: Weather Training FROM SCRATCH — GeoPartLoss Pipeline
# =============================================================================
# Copy to Kaggle notebook → Run All → Done
#
# Trains SPDGeo-DPEA-MAR from scratch using the NEW GeoPartLoss pipeline
# (4-group adaptive uncertainty weighting, IMG_SIZE=448) with WeatherPrompt's
...
#   - EXP53: IMG=336, 12 separate loss components with fixed lambdas
#   - EXP57: IMG=448, GeoPartLoss (4-group adaptive uncertainty weighting)
#            Same as the new no-weather best (98.4% R@1) — NO checkpoint loaded
#
# Kaggle Data Sources:
...
class Config:
    ...
    # --- Model architecture (GeoPartLoss pipeline, IMG=448) ---
    IMG_SIZE        = 448   # NEW vs EXP53 (was 336)
    N_PARTS         = 8
    PART_DIM        = 256
    EMBED_DIM       = 512
    TEACHER_DIM     = 768
    ...
    NUM_EPOCHS      = 120        # Full training (same as new best)
    P_CLASSES       = 16
    K_SAMPLES       = 4
    LR              = 3e-4       # Full LR (not fine-tune)
    BACKBONE_LR     = 3e-5
    WEIGHT_DECAY    = 0.01
    WARMUP_EPOCHS   = 5
    USE_AMP         = True
...
class GeoPartLoss(nn.Module):
    ...
    def forward(self, d_out, s_out, labels, model, teacher, ema, drone, sat, alt_idx, alt_norm, epoch, device):
        ...
        for i, (L, name) in enumerate(zip(groups, group_names)):
            ...
            gw[name] = precision.item(); ld[f'G_{name}'] = L.item()
...
class OnlineWeatherTrainDataset(Dataset):
    def __init__(self, sues_root, drone_transform=None, sat_transform=None, weather_augmenters=None):
        self.drone_transform = drone_transform; self.sat_transform = sat_transform
...
def build_satellite_gallery(model, sues_root, device, transform):
    ...
    for loc in all_locs:
        ...
        else: sat_labels.append(-1000 - dc); dc += 1
    sat_feats = []
    model.eval()
    with torch.no_grad():
...
def main():
    ...
    print("=" * 65)
    print("  EXP57: Weather Train FROM SCRATCH — GeoPartLoss Pipeline")
    print(f"  IMG_SIZE: {CFG.IMG_SIZE} | Epochs: {CFG.NUM_EPOCHS} | Device: {DEVICE}")
    print(f"  Loss: GeoPartLoss (4-group adaptive) | RECON_WARMUP: {CFG.RECON_WARMUP}")
    print("=" * 65)
    ...
    # Teacher and EMA
    teacher = None
    try: teacher = DINOv2Teacher().to(DEVICE); teacher.eval(); print("  DINOv2 ViT-B/14 teacher loaded.")
    ...
    for epoch in range(1, CFG.NUM_EPOCHS + 1):
        ...
        for i, pg in enumerate(optimizer.param_groups):
            base = [CFG.BACKBONE_LR, CFG.LR, CFG.LR][i]; pg['lr'] = base * lr_scale
        ...
        if epoch % CFG.EVAL_INTERVAL == 0 or epoch == CFG.NUM_EPOCHS:
            ...
            if avg_adv_r1 > best_avg_r1:
                ...
                torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(),
                           ...
                           os.path.join(CFG.OUTPUT_DIR, 'exp57_weather_scratch_online_best.pth'))
    ...
    print(f"\n{'='*65}")
    print(f"  EXP57 Complete — Best avg_adv_r1: {best_avg_r1:.2f}%")
    print(f"  Checkpoint: {CFG.OUTPUT_DIR}/exp57_weather_scratch_online_best.pth")
    print(f"  → Run EXP55 with EXPERIMENT_NAME='EXP55_ZeroShot_EXP57' to evaluate")
    print(f"{'='*65}")

    run_summary = {
        'experiment': 'EXP57_WeatherScratch_GeoPartLoss',
        'img_size': CFG.IMG_SIZE,
        'timestamp': {'start': train_start, 'end': datetime.datetime.now().isoformat()},
        'best_avg_adv_r1': best_avg_r1,
        'config': {k: v for k, v in vars(CFG).items() if not k.startswith('_')},
    }
    with open(os.path.join(CFG.OUTPUT_DIR, 'exp57_summary.json'), 'w') as f:
